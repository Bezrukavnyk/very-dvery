(() => {
    ...
    function initSliders() {
        if (document.querySelector(".review__block")) new core(".review__block", {
    ...
    let showmoreWrap = document.querySelectorAll(".tabs__body--catalog");
    let dataItem = null;
    showmoreWrap.forEach((item => {
        if (window.innerWidth > 968) dataItem = item.getAttribute("data-showmore-content"); else dataItem = item.getAttribute("data-showmore-content-mobile");
        let itemCards = item.querySelectorAll(".catalog__card");
        let count = 0;
        itemCards.forEach((cards => {
            count += 1;
            if (count <= dataItem) cards.setAttribute("data-visible", true); else cards.setAttribute("data-visible", false);
        }));
    }));
    let hidenBlock = document.querySelectorAll('div[data-visible="false');
    hidenBlock.forEach((hidden => {
        hidden.classList.add("hidden");
    }));
    let btnMore = document.querySelectorAll(".block__more");
    btnMore.forEach((btn => {
        btn.addEventListener("click", (() => {
            btn.classList.toggle("_active");
            let btnParrent = btn.parentNode;
            let includeHiddenCards = btnParrent.querySelectorAll('div[data-visible="false');
            includeHiddenCards.forEach((item => {
                item.classList.toggle("hidden");
            }));
        }));
    }));
    window.addEventListener("click", (e => {
        let popupSucces = document.querySelector(".popup__succes--form");
        if (popupSucces.classList.contains("_show")) if (e.target.classList.contains("popup__close--succes") || e.target.classList.contains("popup__succes--form")) popupSucces.classList.remove("_show");
    }));
